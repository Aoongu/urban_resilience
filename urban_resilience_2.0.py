...
def cluster_cities(embeddings, city_names, n_clusters=4):
    ...
    plt.savefig(os.path.join(OUTPUT_DIR, "city_clustering_tsne.png"), dpi=300)
    plt.close()
...
def plot_bottleneck_map(G, top_df, city_name, mode):
    ...
    plt.axis('off')
    plt.savefig(os.path.join(fig_dir, f"{city_name}_{mode}_bottlenecks.png"), dpi=300)
    plt.close()
...
def plot_coupled_comparison(result, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(fig_dir, f"{city}_coupled_vs_single.png"), dpi=400)
    plt.close()

# ===================== 网络可视化函数 =====================
def plot_network(G, city_name, mode='bus', layout='geo', save=True):
    """
    绘制单个城市的公交或地铁网络拓扑图
    G: networkx.Graph
    city_name: 城市名
    mode: 'bus' 或 'metro'
    layout: 'geo' 使用经纬度坐标, 'spring' 使用力导向布局
    """
    if G is None or G.number_of_nodes() == 0:
        print(f"{city_name} {mode} 网络为空，跳过绘图")
        return

    plt.figure(figsize=(12, 10))

    if layout == 'geo' and all('x' in G.nodes[n] for n in G.nodes()):
    ...
    node_size = [10 + 3 * degrees[n] for n in G.nodes()]
    nx.draw_networkx_nodes(G, pos, node_size=node_size, node_color='steelblue', alpha=0.8)

    # 标注少数高度数节点（可选）
    high_deg_nodes = sorted(degrees, key=degrees.get, reverse=True)[:5]
    labels = {n: n for n in high_deg_nodes}
    nx.draw_networkx_labels(G, pos, labels, font_size=8, font_color='darkred')

    plt.title(f"{city_name} {mode.upper()} Network (Nodes: {G.number_of_nodes()}, Edges: {G.number_of_edges()})")
    plt.axis('off')
    if save:
        plt.savefig(os.path.join(OUTPUT_DIR, f"{city_name}_{mode}_network.png"), dpi=400, bbox_inches='tight')
        plt.close()
    else:
        plt.show()


def plot_multilayer_network(G_multi, city_name, save=True):
    """
    绘制公交-地铁耦合网络，用颜色区分层
    """
    if G_multi is None:
    ...
    # 分离层节点
    bus_nodes = [n for n, d in G_multi.nodes(data=True) if d.get('layer') == 'bus']
    metro_nodes = [n for n, d in G_multi.nodes(data=True) if d.get('layer') == 'metro']
    inter_edges = [(u, v) for u, v, d in G_multi.edges(data=True) if d.get('layer') == 'inter']
    bus_edges = [(u, v) for u, v, d in G_multi.edges(data=True) if d.get('layer') == 'bus']
    metro_edges = [(u, v) for u, v, d in G_multi.edges(data=True) if d.get('layer') == 'metro']

    # 使用地理坐标（如果存在）
    pos = {}
    for n in G_multi.nodes():
        if 'x' in G_multi.nodes[n] and 'y' in G_multi.nodes[n]:
            pos[n] = (G_multi.nodes[n]['x'], G_multi.nodes[n]['y'])
    if len(pos) == 0:
        pos = nx.spring_layout(G_multi, seed=42)

    # 绘制各层边
    nx.draw_networkx_edges(G_multi, pos, edgelist=bus_edges, edge_color='lightblue', alpha=0.3, width=0.3)
    nx.draw_networkx_edges(G_multi, pos, edgelist=metro_edges, edge_color='salmon', alpha=0.6, width=0.8)
    nx.draw_networkx_edges(G_multi, pos, edgelist=inter_edges, edge_color='green', alpha=0.5, style='dashed', width=0.5)

    # 绘制节点
    nx.draw_networkx_nodes(G_multi, pos, nodelist=bus_nodes, node_color='blue', node_size=5, alpha=0.5, label='Bus')
    nx.draw_networkx_nodes(G_multi, pos, nodelist=metro_nodes, node_color='red', node_size=20, alpha=0.9, label='Metro')

    plt.title(f"{city_name} Multilayer Network (Bus + Metro)")
    plt.legend()
    plt.axis('off')
    if save:
        plt.savefig(os.path.join(OUTPUT_DIR, f"{city_name}_multilayer_network.png"), dpi=400, bbox_inches='tight')
        plt.close()
    else:
        plt.show()


def plot_typical_cluster_networks(city_graphs, cluster_labels, city_names, output_dir):
    """
    从每个聚类中选一个代表城市，绘制其网络图，形成典型形态示例图
    """
    # 将聚类标签与城市对应
    cluster_dict = {}
    for city, label in zip(city_names, cluster_labels):
        if label not in cluster_dict:
            cluster_dict[label] = []
        cluster_dict[label].append(city)

    # 对每个聚类选第一个城市为代表
    for label, cities in cluster_dict.items():
        rep_city = cities[0]
        G = city_graphs.get(rep_city)
        if G is None:
            continue
        plt.figure(figsize=(10, 8))
        pos = {n: (G.nodes[n]['x'], G.nodes[n]['y']) for n in G.nodes() if 'x' in G.nodes[n]}
        if len(pos) == 0:
            pos = nx.spring_layout(G, seed=42)
        nx.draw_networkx_edges(G, pos, alpha=0.3, edge_color='gray', width=0.3)
        degrees = dict(G.degree())
        node_size = [5 + 2 * degrees[n] for n in G.nodes()]
        nx.draw_networkx_nodes(G, pos, node_size=node_size, node_color='steelblue', alpha=0.7)
        plt.title(f"Cluster {label} Representative: {rep_city} (Nodes: {G.number_of_nodes()}, Edges: {G.number_of_edges()})")
        plt.axis('off')
        plt.savefig(os.path.join(output_dir, f"cluster_{label}_rep_{rep_city}.png"), dpi=400, bbox_inches='tight')
        plt.close()
# ===================== 6. 主流程 =====================
def main():
    print("=" * 50)
    ...
    for city in available_cities:
        G_bus, _ = build_graph_from_shapefile(city, mode='bus')
        if G_bus:
            plot_network(G_bus, city, mode='bus', layout='geo')

    # 若有地铁数据，绘制地铁网络及耦合网络
    for city in metro_cities:
        G_metro, _ = build_graph_from_shapefile(city, mode='metro')
        if G_metro:
            plot_network(G_metro, city, mode='metro', layout='geo')
        G_multi = build_multilayer_graph(city)
        if G_multi:
            plot_multilayer_network(G_multi, city)
    # 第三步：韧性模拟（并行 + 缓存）
    print("开始韧性评估（并行 + 缓存）...")
